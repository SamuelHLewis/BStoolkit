...
parser.add_argument('-b', '--bowtie1', type=str, help='bowtie1 piRNA alignment file (bam format)')
...
# bowtie1 piRNA file parsing
if args.bowtie1 is not None:
	bowtie1_file = args.bowtie1
	print("bowtie1 piRNA file is " + bowtie1_file)
else:
	print("No bowtie1 piRNA file (-b) specified, so bowtie1 piRNA count will not be generated")
# siRNA file parsing
...
# function to calculate normalized CpG content within each feature of an annotation file
def CpG_content(annotation, fasta):
	...
	# for each feature, calculate the normalized CpG content (to account for GC content)
	CpG_norm = []
	for record in SeqIO.parse("temp.fasta", "fasta"):
		CpG_fraction =  record.seq.count("CG")*2 / len(record.seq)
		GC_content = (record.seq.count("G") + record.seq.count("C")) / len(record.seq)
		if GC_content > 0:
			# calculating normalized CpG content following method of Saxanov et al (2006) https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1345710/#ref1 
			CpG_content_normalized = CpG_fraction / (GC_content/2)**2
		else:
...
# if bowtie1 piRNA file has been specified, count and read in piRNAs
if bowtie1_file != False:
	print("Counting bowtie1-mapped piRNAs")
	system("bedtools coverage -counts -a " + annotation_file + " -b " + bowtie1_file + " > bowtie1_piRNA.count")
	bowtie1_counts = pd.read_table("bowtie1_piRNA.count", header = None)
...
if bowtie1_file != False:
	combined_data["bowtie1_piRNA"] = bowtie1_counts.iloc[:,-1]
